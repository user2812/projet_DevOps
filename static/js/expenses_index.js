...
document.addEventListener("DOMContentLoaded", e => {
    searchFormInput.placeholder = "Search Expenses";
        ...
        .then(result => {
            ...
            if (result.length == 0) {
                ...
                result.forEach(expense => {
                    ...
                    <td>
                            <div class="accordion-button" style="max-width: 400px;" data-bs-toggle="collapse" data-bs-target="#collapse-${ expense.id }" aria-expanded="false" aria-controls="collapse-${ expense.id }">
                                <button type="button" class="btn btn-outline-info btn-sm me-2">Expand</button>
                                <div class="d-inline-block text-truncate" style="max-width: 400px;">${ expense.description }</div>
                            </div>
                            <div class="accordion-item">
                                <div id="collapse-${ expense.id }" class="accordion-collapse collapse" data-bs-parent="#accordion-description">
                                <div class="accordion-body" style="max-width: 400px;">${ expense.description }</div>
                                </div>
                            </div>
                    ...
                    <td>${ expense.date }</td>
                    <td>
                        <button onclick="getExpenseObj('${ expense.id }')" type="button" class="btn btn-outline-primary" data-bs-toggle="modal" data-bs-target="#editExpensesModal">
                            <!-- edit icon -->
                        ...
                        <button onclick="deleteExpenseObj('${ expense.id }')" type="button" class="btn btn-outline-danger" data-bs-toggle="modal" data-bs-target="#deleteExpensesModal">
                            <!-- trash icon -->
