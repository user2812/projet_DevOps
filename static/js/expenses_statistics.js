...
let renderBarChart = (labels, dataInfo) => {
  ...
  const config = {
    ...
    data: data,
    options: {
      title: {display: false, text : "Expenses (by month)" },
      scales: {
...
const getBarChart = (mths) => {
  if (mths > 0) {
    ...
    fetch(`/expenses/expenses-amount-summary/${mths}`)
    .then(response => response.json())
    .then(result => {

      const keys = Object.keys(result.expenses_amount_data).reverse();
      const values = Object.values(result.expenses_amount_data).reverse();
...
const getPieChart = (mths) => {
  if (mths > 0) {
    ...
    fetch(`/expenses/expenses-category-summary/${mths}`)
    .then(response => response.json())
    .then(result => {

      const keys = Object.keys(result.expenses_category_data);
      const values = Object.values(result.expenses_category_data);
